...
**Change log (V8):**
- **2026-06 (SW v8-v12):** Mobile bottom sidebar now shrinks to fit — all 10 group icons fit without horizontal scrolling down to 320px screens. Items became shrinkable (`flex:0 1`, `min-width` floor 24px), gap 2→1px, separator margins tightened; ≤768px: 34px items/16px icons, ≤480px: 30px items/14px icons.
- **2026-06 (SW v8-v11):** Business-logic validations added to all forms — validation toolkit (`V` validators + `V.run` field rules with red-highlight + toast), enhanced `saveDrawer` (required + numeric min/max + per-form business rules), cross-entity rules: booking unit-availability & advance≤price, customer down-payment≤value, leave balance & date order, payroll deduction≤basic+allowance, payment≤outstanding dues (3 payment forms), duplicate plot ID, floors 1-60/units-per-floor 1-12 for properties; phone/email/amount/rating/rate rules across CRM, HR, Finance, Procurement, Contractor, Supplier, Ticket, Worker, Job, Tax forms; inline-edit numeric guards; CSV import numeric min/max + required-value validation; fixed pre-existing `showModal` bug (Create/Edit buttons never executed their actions).
