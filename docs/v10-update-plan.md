...
- **Phase 6 delivered** (2026-08, `86c7e39`): **Customer Portal** at `https://rem.18.142.98.150.sslip.io/portal` — mobile-first login (customer email + `portal123`), dashboard with dues/invoices/payment history, Pay button per invoice → method chooser (bKash/Nagad) → sandbox checkout → callback runs the full payment ripple server-side (invoice status, dues ledger, cash txn, audit). bKash (tokenized checkout) + Nagad adapters ready: set `PAYMENT_MODE=live` + merchant env vars for production. Verified E2E: portal login, bad-password 401, bKash PAY-013 + Nagad PAY-014, INV-002 Overdue→Paid, dues 10L→0, idempotent callback, staff API unaffected, zero JS errors.
- **Remaining:** open Lightsail 443; go live with real bKash/Nagad merchant credentials; optional MARS data migration (Alternative D).
