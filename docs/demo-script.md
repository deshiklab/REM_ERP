...
5. Check offline: DevTools → Network → Offline → reload → app still renders (SW `rem-erp-v9-v8`).
...
3. App reloads fully from the service worker (`rem-erp-v9-v8`) — dashboard, charts, data all render.
...
- [ ] Offline reload served by SW `rem-erp-v9-v8`
